...
def on_submit(doc, method):
	...
	new_payment_entry.reference_no = doc.idr_sales_invoice_payment_reference_no or "-"
...
def on_cancel(doc, method):
	payment_entry = frappe.get_doc("Payment Entry", doc.idr_payment_entry_reference)
	payment_entry.cancel()
	frappe.db.commit()

	frappe.delete_doc("Payment Entry", payment_entry.name)
	frappe.msgprint(_("Payment entry " + doc.idr_payment_entry_reference + " deleted."))
